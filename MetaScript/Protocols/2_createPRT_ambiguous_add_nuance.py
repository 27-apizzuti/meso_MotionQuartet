...
for itersu, su in enumerate(SUBJ):
    for iterse in SESS:
        ...
        for r in range(0, N_RUNS):
            ...
            # Find GAP
            temp_array = np.asarray(on_off)
            s_gap = temp_array[:-1, 1]
            e_gap = temp_array[1:, 0]
            gap = s_gap - e_gap
            
            # Trial gap
            idx = gap !=0
            idx =np.argwhere(idx)
            new_onoff = np.zeros([np.shape(idx)[0], 2])
            for it in range(0, np.shape(idx)[0]):
                new_onoff[it, 0] = temp_array[int(idx[it]), 1]
                new_onoff[it, 1] = temp_array[int(idx[it])+1, 0]
