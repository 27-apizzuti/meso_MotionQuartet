...
the scanning session for 4.5 min [270 volumes]. By using Siemens online GLM we are able
to positionate correctly the functional slab. This process is done only during
the first session.

Sequence used: CMRR Sequence, GE-BOLD, TR=1s, res=2 iso mm.

Block design: moving dots (10s), static dots (10s). [13 repetitions]
7 target are showed.

The .npz file that is used here is: MtLoc_MQ_4min_4onlineGLM.npz
...
CONDITIONS_FILE = "MtLoc_MQ_4min_4onlineGLM.npz" # change if you want different conditions
...
PixW = 1920#1440.0  # [1920.0] in scanner
PixH = 1200#900.0  # [1200.0] in scanner
...
# Load .npz file containing conditions, durations, and targets
filename = os.path.join(str_path_parent_up, 'Conditions', CONDITIONS_FILE)
