        ...
        for tas in TASK:
            ...
            for path_run in runs:
                ...
                if temp == 'prf00':
                    ...
                    nifti_file = os.path.join(path_in, "topup_prf", "{}_sess-0{}_PA.nii.gz".format(su, se))
                    nii = nb.load(nifti_file) # // We take the header from here
                else:
                    # // Load nifti (created for topup)
                    nifti_file = os.path.join(path_in, "topup_prf", "{}_sess-0{}_AP.nii.gz".format(su, se))
                    nii = nb.load(nifti_file) # // We take the header from here

                # Find FMR and create output name
                PATH_TOPUP = os.path.join(path_run, 'topup_prf')
