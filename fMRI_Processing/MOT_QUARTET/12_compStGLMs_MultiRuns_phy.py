...
for si in SUBJ:
    ...
    for se in SESS:
        ...
        PATH_OUT = os.path.join(STUDY_PATH, si, 'derivatives', 'func', 'Stats', 'Phy')
    ...
    docVMR.z_transform_studies = False
    docVMR.z_transform_studies_baseline_only = False
    docVMR.separate_subject_predictors = False
    docVMR.separate_study_predictors = False
    docVMR.psc_transform_studies = False
    docVMR.save_multistudy_glm_definition_file(os.path.join(PATH_OUT, '{}_task-phy_VTC_N-{}_FFX_AR-2_3preds.mdm'.format(si, ntot)))
    docVMR.compute_multistudy_glm()
    docVMR.save_glm(os.path.join(PATH_OUT, '{}_task-phy_VTC_N-{}_FFX_AR-2_3preds.glm'.format(si, ntot)))
    docVMR.show_glm()
        # doc.close()
