    ...
    for hem in HEMI:
        for ro in ROI:
            for co in CONDITION:
                ...
                subj_dict["Horizontal"]["Metric"] = metr[idx]
                subj_dict["Horizontal"]["betas"] = betas_hor[idx]

                subj_dict["Vertical"]["Metric"] = metr[idx]
                subj_dict["Vertical"]["betas"] = betas_ver[idx]

                np.save(os.path.join(PATH_OUT, "{}_depth_vs_{}_{}_{}_BETAS_PSC.npy".format(su, co, hem, ro)), subj_dict)
